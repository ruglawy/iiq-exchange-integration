...
########################## PROXY EMAIL ADDRESS ADDITION #############################
# Used if Active Directory's domain is different than that of your actual domain used in emails
# And you want to use your actual domain instead of AD's
# You create a proxy email address with your actual domain and add it to the user
# THIS IS NOT NEEDED IF YOUR AD's DOMAIN IS THE SAME AS YOUR ACTUAL DOMAIN
function Ensure-ActualDomainProxy {
    ...
    try {
        ...
        $proxy   = "$local@actual.domain" # <--- Change this domain according to your actual domain

        $exists = $mbx.EmailAddresses | Where-Object {
            $_.ToString().ToLower() -eq "smtp:$proxy"
        }

        if (-not $exists) {
            Set-Mailbox $User -EmailAddresses @{add="smtp:$proxy"}
            Log "PROXY ADDED for $User : $proxy"
        }
        else {
            Log "PROXY already exists for $User"
        }
    }
    catch {
        Log "ERROR adding proxy for $User : $_"
    }
}
#####################################################################################
...
if ($inGroup)
{
    ...
    if ($recipient -and $recipient.RecipientTypeDetails -eq "UserMailbox") {
        ...
        ############### ADD PROXY EMAIL ADDRESS
        Ensure-ActualDomainProxy -User $User
